...
public class ExceptionHandlerAdvice {
    ...
    public ApiResult<?> exception(HttpServletResponse response, BindException exception, HandlerMethod handler)
            throws IOException {
    ...
    public ApiResult<?> exception(HttpServletResponse response, MethodArgumentNotValidException exception,
    ...
    public ApiResult<?> exception(BizException bizException, HandlerMethod handlerMethod, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        return ApiResult.getFail(bizException.getErrorCode(), message, bizException.getData());
    }

    @ResponseBody
    @ExceptionHandler({Error.class, Exception.class, Throwable.class, RuntimeException.class})
    public ApiResult<?> exception(HttpServletResponse response, Throwable exception, HttpServletRequest request) throws IOException {
        log.error("System error:", exception);
        response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
        String message = messageHelper.getMessage(ErrorCode.SYS_ERROR);
        return ApiResult.getFail(ErrorCode.SYS_ERROR, message, null);
    }

    private ApiResult<?> getValid(BindingResult bindingResult) {
        ...
        return ApiResult.getFail(ErrorCode.SYS_PARAMS_VALID, message, data);
    }
